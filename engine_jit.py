...
def train_one_epoch(model, model_without_ddp, data_loader, optimizer, device, epoch, log_writer=None, args=None):
    ...
    for data_iter_step, (x, labels) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if log_writer is not None:
            # One monotonic global step for all W&B metrics.
...
def evaluate(model_without_ddp, args, epoch, batch_size=64, log_writer=None, log_step=None):
    ...
    for i in range(num_steps):
        ...
        # Log a small generated batch for qualitative monitoring in W&B.
        if log_writer is not None and i == 0 and args.wandb_num_vis > 0:
            num_vis = min(args.wandb_num_vis, sampled_images.size(0))
            captions = [f"class_{int(labels_gen[idx].item())}" for idx in range(num_vis)]
            step = int(epoch if log_step is None else log_step)
            log_writer.add_images('samples/generated', sampled_images[:num_vis], step, captions=captions)
    ...
    # Compute FID/IS on the main process even when W&B logging is disabled.
    if misc.get_rank() == 0:
        ...
        step = int(epoch if log_step is None else log_step)
        if log_writer is not None:
            log_writer.add_scalar('fid{}'.format(postfix), fid, step)
            log_writer.add_scalar('is{}'.format(postfix), inception_score, step)
